<? 
	...
	$artId=@$_GET["Id"];
	if($artId==""){
		exit("<script>window.location.href='article-list.php';alert('未选择文章！');</script>");
	}
	$sql="select * from `my_article` where `Id`=".$artId;
...
<title>我的博客 - 文章详情 - <? echo $result_arr["title"]?></title>
</head>
...
<body>
	<div class="body">
        ...
        <div class="main clearfix">
            <div class="main-left">
                <div class="main-page">
                    当前位置：&nbsp;<a href="">首页</a>&nbsp;>&nbsp;<a href="">文章</a>&nbsp;>&nbsp;<? echo $result_arr["title"]?>
                </div>
                <div class="left-art">
                    ...
                    <?
						$artId_p=$artId-1;
						$sql_p="select * from `my_article` where `Id`=".$artId_p;
					...
					?>
                    	<li>上一篇：<a href="article.php?Id=<? echo $result_arr_p["Id"]?>" target="_blank"><? echo $result_arr_p["title"]?></a></li>
					<? }
						$artId_n=$artId+1;
						$sql_n="select * from `my_article` where `Id`=".$artId_n;
						$result_n=mysql_query($sql_n);
						$rows_num_n=mysql_num_rows($result_n);
						if($rows_num_n=="0"){ ?>
                        <li>下一篇：没有了！</li>
                    <? }else{
						$result_arr_n=mysql_fetch_array($result_n);
					?>
                        <li>下一篇：<a href="article.php?Id=<? echo $result_arr_n["Id"]?>" target="_blank"><? echo $result_arr_n["title"]?></a></li>
                    <? }?>
                    </ul>
                    <div class="recomand-box">
                        ...
                        <form  method="post" action="recomand_deal.php">
                            ...
                            <input type="hidden" value="http://127.0.0.1:7517/my%20BLOG/article.php?Id=<? echo $artId?>" name="recomand_address" />
                            <div><textarea rows="3" id="" name="content" autocomplete="off" placeholder="请输入你的评论吧！" class="recomand"></textarea></div>
                            <div class="sub-lab"><!--<input type="checkbox" class="checkbox" checked="" value="1" name="" id="n-name" /><label for="n-name">匿名评论</label>--><span><input type="submit" class="submit" value="提交" /></span></div>
                        </form>
                    </div>
                    <ul class="recomands-box">
                        ...
                        <!--循环体开始-->
                        <? 
							$sql_rmd="select * from `my_recomand` where (`recomand_address`='http://127.0.0.1:7517/my%20BLOG/article.php?Id=".$artId."' and `issure`=1) order by addtime";
							//echo $sql_rmd;
							...
							$sql_art_rmd="select * from `my_recomand` where (`recomand_address`='http://127.0.0.1:7517/my%20BLOG/article.php?Id=".$artId."' and `issure`=1) order by addtime desc limit ".$offset.",".$pagesize;
							//echo $sql_art_rmd;
